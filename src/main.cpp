...
int main(int argc, const char* argv[]) {
  ...
  // Iterator
  // Iterator here functions pretty much the same as those of other containers in std.
  std::cout << "--- ITERATOR ---\n";
  {
    List<int> list{ 1,2,3 };
    op<int>("list: ", list);
    
    // Returns an iterator to the first element of the list, i.e. iterator to "1".
    // If the list is empty, returns end iterator (see below).
    List<int>::iterator begin = list.begin();
    List<int>::const_iterator cbegin = list.begin();
    
    // Returns an iterator to the end of the list.
    // Mind u that this is not at the last element; rather, at an arbitrary element AFTER the last element.
    // So, `--list.end()` actually points to the last element of the list.
  ...
  // Methods
  {
    ...
    // Removals
    // `list` in here is constantly refilled (due to the nature of this section :v,
    // so keep an eye out.
    std::cout << "\n--- REMOVALS ===\n";
    {
      list = { 1,2,3 };
      ...
      //
      // Removes all elements satisfying some conditions (i.e. when the boolean function returns true).
      // Returns the new size of the list.
      //
      // Returns 3
      // list: 4 3 5
      ...
      op<int>("list: ", list);
      // Syntax: remove_if(function<bool(const T&)> func, iterator first, iterator last);
      //
      // Removes in a specified range [first, last).
      //
      // Notice how `4` doesn't get removed?
      // list: 4 5
      list.remove_if([](const int& x) { return x <= 4; }, ++list.begin(), list.end());
      op<int>("list: ", list);
    }
    
    
    // Resizing
    // This is the combination of both insertions and removals, ensuring that the resulting list
    // has the length of a speicified value.
    std::cout << "\n--- RESIZING ---\n";
    {
      list = {};
      // Resize the list to a specified length.
      // If more elements need to be added in, default value of current data type will be used (0 for int).
      //
      // list: 0 0 0
      list.resize(3);
      op<int>("list: ", list);
    ...
    std::cout << "\n--- ASSIGNING ---\n";
    {
      ...
      // Aside from `assign` methods, `=` operator functions in the same manner also.
      //
      // list: 3 2 1
    ...
    //
    // Each of the methods below comes with another version that allows performing on a the
    // range of [first, last) (rather than on the whole list).
    // Ex: list.find(<value>, <first_iterator>, <last_iterator>);
    std::cout << "\n--- UTILITIES ---\n";
    {
      list = { 2,2,3 };
      ...
      std::cout << "find_if: " << (*list.find_if([](const int& x) { return x != 2; })) << '\n';
      ...
      //
      // 3
      std::cout << "count_if: " << list.count_if([](const int& x) { return x > 1; }) << '\n';
      ...
      //
      // Checks if there is AT LEAST ONE element satisfying some conditions (i.e. when boolean function returns true).
      ...
      //
      // false
      std::cout << "none_of: " << list.none_of([](const int& x) { return x == 3; }) << '\n';
      ...
      // count == 2.
      int count = 0;
      list.for_each([&](const int& x) { if (x % 2 == 0) ++count; });
      std::cout << "for each: " << count << '\n';
      
      // Syntax: map<T>(function<TT(const T&)> func));
      //
      // Applies a function to every element, then gathers all returned values to another list.
      // Returns the new list.
      //
      // l2: c c d
      List<char> l2 = list.map<char>([](const int& x) { return x + 'a'; });
      std::cout << "map: "; op("l2: ", l2);
      ...
      //
      // product == 2 * 2 * 3 = 12
      int product = list.reduce([](const int& x, const int& y) { return x * y; }, 1);
